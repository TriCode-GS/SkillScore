...
export async function autenticarGestor(credentials: LoginCredentials): Promise<LoginResponse> {
  const baseUrl = getBaseUrl()
  let url: URL
  
  try {
    url = new URL(`${baseUrl}/logins/autenticar/gestor`)
  } catch (error) {
    throw new Error(`URL inválida: ${baseUrl}/logins/autenticar/gestor`)
  }
